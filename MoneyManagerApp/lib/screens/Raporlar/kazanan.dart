...
class KazananSutunGrafik extends StatefulWidget {
  @override
  _KazananSutunGrafikState createState() => _KazananSutunGrafikState();
}

class _KazananSutunGrafikState extends State<KazananSutunGrafik> {
            ...
            child: Container(
              ...
              child: synn.SfCartesianChart(
                title: synn.ChartTitle(
                    text:
                        "Ailedeki Kazanan Fert Sayısına Göre Harcama Tahmini"),
  ...
  Future getDefaultLineSeries() async {
    String xmlString = await DefaultAssetBundle.of(context)
        .loadString("assets/ai/kazanan.xml");
    var raw = xml.XmlDocument.parse(xmlString);
    var elements = raw.findAllElements("doc");

    var v111 = elements.map((element) {
      return Verilerim(
          "1", double.parse(element.findElements("tahmin1").first.text));
    }).toList();
    var v222 = elements.map((element) {
      return Verilerim(
          "2", double.parse(element.findElements("tahmin2").first.text));
    }).toList();
    var v333 = elements.map((element) {
      return Verilerim(
          "3", double.parse(element.findElements("tahmin3").first.text));
    }).toList();
    var v444 = elements.map((element) {
      return Verilerim(
          "4", double.parse(element.findElements("tahmin4").first.text));
    }).toList();
